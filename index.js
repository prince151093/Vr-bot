...
async function handleActivityCommand(message) {
  ...
  if (action === "on") { config.enabled = true; state.lastHumanAt = Date.now(); return message.reply("🟢 Community Activity is **ON**."); }
  if (action === "off") { config.enabled = false; return message.reply("🔴 Community Activity is **OFF**."); }
  if (action === "reset") {
